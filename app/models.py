...
student_project_association_table = db.Table(
        'StudentProjectAssociation', 
        db.Model.metadata,
        db.Column('student_id', db.Integer, db.ForeignKey('users.id')),
        db.Column('project_id', db.Integer, db.ForeignKey('projects.id'))
)

mentor_project_association_table = db.Table(
        'MentorProjectAssociation', 
        db.Model.metadata,
...
class User(UserMixin, db.Model):
    ...
    #### STUDENT
    milestones_completed = db.Column(db.Integer)
    ...
    def generate_fake(count=100):
        ...
        new_roles = ['mentor', 'student', 'teacher', 'parent']
        schools = []
...
# ---------------------------------sq------------------------------------------
    ...
    #     """ i will start with the ability to 'advise' specific projects.
    #     later i will implement the 'mentoring' relationship which won't 
    #     require a project. """
    #     if not project.is_advising(self):
    #         m = Advising(mentor_id=self.id, student=student.id)
    #         db.session.add(m)

    # def is_advising(self, project):
    #     return self.projects.filter_by(
    #             project_id=project.id).first is not None
